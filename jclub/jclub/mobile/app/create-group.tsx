...
export default function CreateGroupScreen() {
    ...
    // If coming from club screen, club is pre-selected and not editable
    const isFromClub = !!clubId;
    ...
    // Auto-select club when clubId is available
    useEffect(() => {
        if (clubId && myClubs.length > 0) {
            // First try to find in joined clubs
            let targetClub = myClubs.find((c: any) => c.id === clubId);
            
            // If not found and we're from club screen, fetch the club directly
            if (!targetClub && isFromClub) {
                // Try to get from local storage or fetch directly
                api.getClub(clubId as string).then((clubData: any) => {
                    if (clubData) {
                        setSelectedClub(clubData);
                    }
                }).catch(() => {
                    console.log("Could not fetch club directly");
                });
            } else if (targetClub) {
                setSelectedClub(targetClub);
            }
        }
    }, [clubId, myClubs, isFromClub]);

    const loadMyClubs = async () => {
        try {
            // First get current user
            const userData = await api.getProfile();
            console.log("[CreateGroup] User data:", userData);
            setCurrentUser(userData);
            
            // Get clubs that user has CREATED (filter = "created")
            const clubs = await api.getClubs(1, 100, '', 'created');
            console.log("[CreateGroup] My created clubs:", clubs);
            
            // Show only clubs created by this user
            setMyClubs(clubs || []);
        } catch (e) {
    ...
    const handleCreate = async (status: 'draft' | 'published') => {
        ...
        if (!selectedClub) {
            alert("Silakan pilih club terlebih dahulu");
            return;
        }
        ...
        <View style={styles.container}>
            ...
            <SafeAreaView edges={['top']} style={styles.headerSafeArea}>
                <View style={styles.header}>
                    ...
                    <Text style={styles.headerTitle}>
                        {isFromClub && selectedClub ? `Buat Jadwal - ${selectedClub.name}` : 'Buat Jadwal'}
                    </Text>
                    <View style={{ width: 40 }} />
                </View>
            </SafeAreaView>
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.formCard}>
                    ...
                    {isFromClub ? (
                        // When coming from club screen, show pre-selected club (not editable)
                        <View style={[styles.input, { backgroundColor: '#F5F5F5' }]}>
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Ionicons name="albums-outline" size={20} color={PRIMARY_GREEN} style={{ marginRight: 8 }} />
                                <Text style={{ color: '#000', fontWeight: '600' }}>
                                    {selectedClub?.name || 'Loading...'}
                                </Text>
                            </View>
                        </View>
                    ) : (
                        // From home screen - allow club selection
                        <>
                            {myClubs.length > 0 ? (
                                <TouchableOpacity 
                                    style={styles.input}
                                ...
                                >
                                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                            <Ionicons name="albums-outline" size={20} color={PRIMARY_GREEN} style={{ marginRight: 8 }} />
                                            <Text style={{ color: selectedClub ? '#000' : '#999' }}>
                                                {selectedClub?.name || 'Pilih club...'}
                                            </Text>
                                        </View>
                                        <Ionicons name={showClubPicker ? "chevron-up" : "chevron-down"} size={20} color="#757575" />
                                    </View>
                                </TouchableOpacity>
                            ) : (
                                <TouchableOpacity 
                                ...
                                >
                                    <Text style={{ color: '#FF9800' }}>Belum punya club. Buat dulu di My Clubs →</Text>
                                </TouchableOpacity>
                            )}
                            
                            {showClubPicker && (
                                <View style={{ marginTop: 8, borderWidth: 1, borderColor: '#E0E0E0', borderRadius: 12, maxHeight: 200 }}>
                                    <ScrollView style={{ maxHeight: 200 }}>
                                        {myClubs.map((club: any) => (
                                            ...
                                            >
                                                <Text style={{ fontWeight: selectedClub?.id === club.id ? '700' : '400' }}>
                                                    {club.name}
                                                </Text>
                                            </TouchableOpacity>
                                        ))}
